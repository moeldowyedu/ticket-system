			...
			<div class="card-body">
				<div class="container-fluid">
					<div class="form-group text-center">
						...
						<div id="t-container" class="d-grid">
							<?php
							$trans = $conn->query("SELECT * FROM transactions where status = 1 AND active = 'on' order by name asc");
							
							while ($row = $trans->fetch_assoc()) :
							?>
								<button type="button" class="btn m-btn transaction-c mb-2" data-name="<?=$row['name']?>" data-tid="<?= $row['id'] ?>" data-symbol="<?= $row['symbol'] ?>" data-displaystatus="<?=$row['displayStatus']?>" data-numberfrom="<?= $row['numberFrom']?>" data-numberto="<?= $row['numberTo']?>"><?= $row['name'] ?></button>
							<?php endwhile; ?>
						</div>
...
<script>
	...
	$('.m-btn').click(function(e) {
		...
		var data = {
			...
			transaction_name: button.data('name'),
		...
		if(button.data('displaystatus')==='on')
		{
			end_load();
			window.show_modal('اختيار الحالة', 'status_dialog.php', 'modal-lg','POST',data);
		}
		else
		{
			processQueue(data);
